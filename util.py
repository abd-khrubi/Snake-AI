...
def compactness(snake):
    ...
    for i in range(len(snake)):
        for j in range(i + 1, len(snake)):
            if (abs(snake[i][0] - snake[j][0]) == 1) and (abs(snake[i][1] - snake[j][1]) == 1):
                compactness += 1
    return (-1) * compactness


def connectivity(snake, board):
    empty = []
...
def fitness(xy1, xy2, snake, board):

    return (0.5 * manhattanDistance(xy1, xy2)) + 0.125 * (
            squareness(snake) + compactness(snake) + connectivity(snake, board)
            + dead_end(snake, board))
